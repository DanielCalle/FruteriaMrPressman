...
public class GUIDevolucion extends JFrame {

	private String[] labels = { "IdVenta", "IdProducto", "Unidades" };
	...
	private void initGUI() {
		...
		JPanel mainPanel = new JPanel(new BorderLayout(10, 10));
			...
			northPanel.add(new JLabel("Devolucion"));
					...
					public void actionPerformed(ActionEvent e) {
						...
						if (create) {
							...
							for(int i = 0; i < out.length; ++i){
								d = Integer.parseInt(out[i]);
								datos.add(d);
							}
							Controlador.getInstance().accion(Events.DEVOLUCION_VENTA, datos);
							} catch (Exception ex) {
