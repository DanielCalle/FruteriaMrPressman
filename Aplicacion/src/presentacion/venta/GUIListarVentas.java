...
public class GUIListarVentas extends JFrame {
	
	private JTable table;
	private String[] columnNames = { "#", "Id", "Fecha", "Precio Total", "IdCliente", "NumLineaVentas"}; 
		...
		JPanel mainPanel = new JPanel(new BorderLayout(10, 10));
				...
				public void mouseClicked(MouseEvent e) {
					if (e.getClickCount() == 2){
						...
						if (column == columnNames.length - 1){
							Controlador.getInstance().accion(Events.BUSCAR_VENTA, ids.get(row));
						}
					}
				}
			});
			...
			button.addActionListener(new ActionListener(){
				...
				public void actionPerformed(ActionEvent e) {
					Controlador.getInstance().accion(Events.LISTAR_VENTA, null);
					toFront();
				}
	...
	public void update (ArrayList<TVenta> res){
		 tableModel.setRowCount(0);
		 ids.clear();
		 for (int i = 0; i < res.size(); i++) {
			 tableModel.insertRow(i, new Object[] 
					 { i, res.get(i).getId(), res.get(i).getFecha().toString(), res.get(i).getPrecioTotal(), res.get(i).getClienteID(), res.get(i).getLineaVentas().size() });
