				...
				button.addActionListener(new ActionListener(){
					...
					public void actionPerformed(ActionEvent e) {
						...
						for (int i = 0; i < labels.length && create; ++i){
							...
							if (textAux.equalsIgnoreCase("")) create = false;
							text += textAux + "\n";
						}
						if (create) {
									...
									if( tV != null){
										...
										if(cond){
											if(unidades > 0){
												...
												//lVEncontrada.setIdProducto(Integer.parseInt(out[0]));
												lVEncontrada.setUnidades(unidades);
												lV.put(id, lVEncontrada);
												tV.setLineaVentas(lV);
												guiVenta.setVenta(tV);
												guiVenta.update(Events.RES_MODIFICAR_PRODUCTO_OK, tV);
											}
											else if(unidades == 0){
												lV.remove(id);
												tV.setLineaVentas(lV);
												...
												guiVenta.update(Events.RES_MODIFICAR_PRODUCTO_KO, -3);		
										}
										else
											guiVenta.update(Events.RES_MODIFICAR_PRODUCTO_KO, -2);
									}
									else
										guiVenta.update(Events.RES_MODIFICAR_PRODUCTO_KO, -1);
								}
								else
									guiVenta.update(Events.RES_MODIFICAR_PRODUCTO_KO, -3);
							} catch (Exception ex) {
